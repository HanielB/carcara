...
impl<'e> Elaborator<'e> {
    ...
    pub fn elaborate_with_default_pipeline(
        ...
        use ElaborationPass::*;
        let pipeline = vec![SkoRename, Polyeq, Hole, Eunif, Local, Uncrowd, Reordering];
    ...
    pub fn elaborate_with_stats(
        ...
        for pass in pipeline {
            ...
            current = match pass {
                ...
                ElaborationPass::Eunif => current
                    .mutate::<_, ()>(|_, node, _| match node.as_ref() {
                        ProofNode::Step(s) if s.rule == "g_eunif" => {
                            // TODO: add proper error handling
                            Ok(eunif::g_eunif(self.pool, &mut self.eunif_cc, s).unwrap())
                        }
                        _ => Ok(node.clone()),
                    })
                    .unwrap(),
                ElaborationPass::SkoRename => ProofNodeForest(
                    current
                        .0
                        .iter()
                        .map(|root| sko_rename::elaborate_sko_rename(self.pool, root))
                        .collect(),
                ),
