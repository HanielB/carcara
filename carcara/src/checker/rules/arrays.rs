use super::{assert_eq, get_premise_term, assert_num_premises, assert_clause_len, RuleArgs, RuleResult};
...
pub fn row(RuleArgs { conclusion, premises, .. }: RuleArgs) -> RuleResult {
    ...
    assert_clause_len(conclusion, 1)?;
    let (((a1, ic, _), jc1), (a2, jc2)) = match_term_err!((= (select (store a1 ic e) jc1) (select a2 jc2)) = &conclusion[0])?;
...
pub fn row_contra(RuleArgs { conclusion, premises, .. }: RuleArgs) -> RuleResult {
    assert_num_premises(premises, 1)?;
    let premise = get_premise_term(&premises[0])?;
    let (((a1, ip, _), jp1), (a2, jp2)) = match_term_err!((not (= (select (store a1 ip e) jp1) (select a2 jp2))) = premise)?;
    assert_clause_len(conclusion, 1)?;
...
pub fn ext(RuleArgs { conclusion, pool, .. }: RuleArgs) -> RuleResult {
    Ok(())
}
