...
def scatter(ax, xs, ys, lo, hi, band, xlabel, ylabel, title):
...
def main():
    ...
    scatter(ax, j['a_time'].to_list(), j['c_time'].to_list(), 1e-3, 8e3,
            CHECK_BAND,
    ...
    fig, ax = plt.subplots(figsize=(6.5, 6))
    scatter(ax, j['a_bytes'].to_list(), j['c_bytes'].to_list(), 1e2, 1e10,
            BYTES_BAND,
