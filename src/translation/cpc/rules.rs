...
impl CpcTranslator<'_> {
        ...
        let info = match rule {
            ...
            "cong" | "nary_cong" | "pairwise_cong" => {
                ...
                self.singleton(
                    ...
                    res,
                    "cong",
                    positions[first_non_refl..].to_vec(),
                    Vec::new(),
                )
            }
            "ho_cong" => {
            ...
            "split" => {
                ...
                let aux = self.aux_id(&id);
                let vp2 = self.push_step(aux, vec![not4, not1], "not_not", Vec::new(), Vec::new());
            ...
            "true_intro" => {
                ...
                let aux = self.aux_id(&id);
                let vp1 = self.push_step(aux, vec![eq], "equiv_simplify", Vec::new(), Vec::new());
            ...
            "instantiate" => {
                ...
                let or_term = self.build_op(Operator::Or, vec![not_forall.clone(), res.clone()]);
                let inst_args = Self::list_elements(&step.args[0])
                    .iter()
                    .map(|t| self.convert(t))
                    .collect();
                let aux = self.aux_id(&id);
                let vp1 = self.push_step(aux, vec![or_term], "forall_inst", Vec::new(), inst_args);
                let aux = self.aux_id(&id);
                let vp2 = self.push_step(
                    aux,
            ...
            "alpha_equiv" => {
                ...
                if x_bindings == y_bindings {
                    ...
                    let clash = y_bindings
                        .iter()
                        .any(|(y_name, _)| x_bindings.iter().any(|(x_name, _)| x_name == y_name));
                    if clash {
                        ...
                        for (x_var, fresh_var) in x_bindings.iter().zip(fresh_bindings.iter()) {
                        ...
                        let eq1 =
                            self.build_op(Operator::Equals, vec![lhs.clone(), lhs_renamed.clone()]);
                        let aux = self.aux_id(&id);
    ...
    fn translate_cnf_ite3(&mut self, id: &str, res: Rc<Term>, positive: bool) -> Result<Info> {
        let elements =
            Self::or_elements_owned(&res).ok_or_else(|| TranslationError::InvalidStep {
                id: id.to_owned(),
                rule: if positive {
                    "cnf_ite_pos3"
                } else {
    ...
    fn translate_arith_reduction(&mut self, step: &ProofStep, res: Rc<Term>) -> Result<Info> {
        ...
        let (eq_steps, intro_rule): (Vec<(&str, Vec<Rc<Term>>)>, &str) = match op_term.as_ref() {
            ...
            Term::Op(Operator::Mod, args) => {
                ...
                (
                    vec![(
                        "rare_rewrite",
                        vec![rule_name, args[0].clone(), args[1].clone()],
                    )],
                    "div_intro",
                )
            }
            ...
            Term::Op(Operator::Abs, args) => {
                ...
                let is_int = *self.pool.sort(&arg) == Sort::Int;
                let rule_name = self.new_string(if is_int {
                    "abs-elim-int"
                } else {
                    "abs-elim-real"
                });
        ...
        // constant divisors
        if eq_rule == "refl" && match_term!((= l r) = op_eq).is_none_or(|(l, r)| l != r) {
            log::warn!("unsupported `arith_reduction` equality, using `hole`");
            return Ok(self.hole(step, res));
        }
        ...
        let aux = self.aux_id(&id);
        let intro_position =
            self.push_step(aux, vec![op_intro], intro_rule, Vec::new(), Vec::new());
    ...
    fn translate_skolemize(
        ...
        let premise = premises
            .first()
            .ok_or_else(|| invalid("expected a premise"))?;
        ...
        let cong_term = self.build_op(Operator::Equals, vec![premise_term.clone(), res.clone()]);
        let aux = self.aux_id(&id);
    ...
    fn translate_quant_var_reordering(&mut self, id: &str, res: Rc<Term>) -> Result<Info> {
        ...
        let vp1_term = self.build_op(Operator::Equals, vec![forall_z.clone(), forall_x.clone()]);
        let aux = self.aux_id(id);
        let vp1 = self.push_step(aux, vec![vp1_term], "qnt_rm_unused", Vec::new(), Vec::new());

        let vp2_term = self.build_op(Operator::Equals, vec![forall_x.clone(), forall_z.clone()]);
        let aux = self.aux_id(id);
...
impl CpcTranslator<'_> {
    ...
    fn translate_quant_merge_prenex(&mut self, step: &ProofStep, res: Rc<Term>) -> Result<Info> {
        ...
        loop {
            ...
            let aux = self.aux_id(&id);
            let position =
                self.push_step(aux, vec![eq.clone()], "qnt_join", Vec::new(), Vec::new());
        ...
        if current_eq != res {
            ...
            let rm_unused_term =
                self.build_op(Operator::Equals, vec![rhs.clone(), current.clone()]);
            let aux = self.aux_id(&id);
            ...
            return Ok(self.singleton(id, res, "trans", vec![current_position, symm], Vec::new()));
        }
        Ok(Info {
