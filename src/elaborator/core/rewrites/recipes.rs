...
pub fn rewrite_lemma(b: &mut Builder, name: &str, lhs: &Rc<Term>, rhs: &Rc<Term>) -> Res {
    match name {
        ...
        "eq-refl" => {
            // `(= (= t t) true)`
            let r = refl(b, match_term_err!((= t t) = lhs)?);
            bridge_true(b, r, lhs)
        }
        "eq-symm" => {
            // `(= (= t s) (= s t))`
        ...
        "bool-not-eq-false" => {
            // `(= (not (= t t)) false)`
            let t = match_term_err!((not (= t t)) = lhs)?;
            let t = t.clone();
            let e = eq(b.pool, &t, &t);
            let r = refl(b, &t);
            let nn = nn_intro(b, &e)?;
