...
use super::{Config, FunctionDef, Parser, ParserError, Position, Reserved, Source, Token};
use crate::{
    CarcaraResult,
    ast::{
...
impl Parser<'_, '_> {
    ...
    pub fn parse_cpc_proof(&mut self) -> CarcaraResult<Proof> {
        self.cpc_mode = true;
        // cvc5 prints indexed operators in flat form, e.g. `(extract 3 0 x)`
        let flat_indexed_ops = self.config.allow_higher_order_indexed_ops;
        self.config.allow_higher_order_indexed_ops = true;
        self.insert_cpc_internal_constants();
        let result = self.parse_cpc_proof_inner();
        self.cpc_mode = false;
        self.config.allow_higher_order_indexed_ops = flat_indexed_ops;
        result
    }
    ...
    fn insert_cpc_internal_constants(&mut self) {
        let int = self.pool.add_sort(Sort::Int);
        let real = self.pool.add_sort(Sort::Real);
        let int_to_int = self
            .pool
            .add_sort(Sort::Function(vec![int.clone(), int.clone()]));
        let real_to_real = self
            .pool
            .add_sort(Sort::Function(vec![real.clone(), real.clone()]));

        let bool_sort = self.pool.add_sort(Sort::Bool);
        // (-> Int Int Int Int): the width and the two operands
        let piand_sort = self.pool.add_sort(Sort::Function(vec![
            int.clone(),
            int.clone(),
            int.clone(),
            int.clone(),
        ...
        let root_predicate_sort = self.pool.add_sort(Sort::Function(vec![
            int,
            bool_sort.clone(),
            real.clone(),
            bool_sort,
        ...
        for (name, sort) in constants {
            self.declare_symbol(name.to_owned(), sort);
        }
    }
    ...
    fn parse_cpc_proof_inner(&mut self) -> CarcaraResult<Proof> {
        ...
        while self.current_token != Token::Eof && self.current_token != Token::CloseParen {
            ...
            let (id, command) = match token {
                ...
                Token::ReservedWord(Reserved::StepPop) => {
                    ...
                    if stack.len() < 2 {
                        return Err(self.err(ParserError::UnmatchedStepPop(id), position));
                    }
        ...
        let commands = match stack.len() {
            ...
            _ => {
                return Err(self.err(
                    ParserError::UnclosedSubproof(stack.pop().unwrap().commands[0].id().to_owned()),
    ...
    fn parse_cpc_define(&mut self) -> CarcaraResult<(String, FunctionDef)> {
        ...
        self.state.symbol_table.push_scope();
        for var in &params {
            self.declare_symbol(var.0.clone(), var.1.clone());
        }
    ...
    fn parse_cpc_step(&mut self, position: Position) -> CarcaraResult<ProofStep> {
        ...
        let Some(conclusion) = conclusion else {
            return Err(self.err(ParserError::CpcMissingConclusion(id), position));
        };
    ...
    pub(super) fn parse_cpc_application(&mut self) -> CarcaraResult<Rc<Term>> {
        ...
        match head.as_str() {
            ...
            "@array_deq_diff" => {
                ...
                let Sort::Array(index_sort, _) = array_sort.as_ref() else {
                    return Err(self.err(ParserError::UnsupportedCpcSymbol(head), head_pos));
                };
            ...
            _ => Err(self.err(ParserError::UnsupportedCpcSymbol(head), head_pos)),
        }
    }
    ...
    fn make_cpc_skolem_app(
        &mut self,
        name: &str,
        arg_sorts: Vec<Rc<Sort>>,
        return_sort: Rc<Sort>,
        args: Vec<Rc<Term>>,
    ) -> Rc<Term> {
        let mut sorts = arg_sorts;
        sorts.push(return_sort);
        let func_sort = self.pool.add_sort(Sort::Function(sorts));
    ...
    pub(super) fn parse_cpc_binder(&mut self, binder: Binder) -> CarcaraResult<Rc<Term>> {
        ...
        let Term::Op(Operator::RareList, elements) = list.as_ref() else {
            return Err(self.err(ParserError::ExpectedVarList(list), pos));
        };
        let bindings = elements
            .iter()
            .map(|element| match element.as_ref() {
                Term::Var(name, sort) => Ok((name.clone(), sort.clone())),
                _ => Err(self.err(ParserError::ExpectedVarList(list.clone()), pos)),
            })
