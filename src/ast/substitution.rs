...
impl Substitution {
    ...
    pub fn new(
        ...
        map: impl IntoIterator<Item = (Rc<Term>, Rc<Term>)>,
    ) -> SubstitutionResult<Self> {
        let map: IndexMap<Rc<Term>, Rc<Term>> = map.into_iter().collect();
        for (k, v) in &map {
    ...
    /// Applies the substitution to `term`, like [`Substitution::apply`].
    ///
    /// The name is kept for callers that want to skip cache maintenance; with the lazy,
    /// generation-tagged cache, `insert` no longer scans the cache, so plain `apply` is already
    /// cheap for the build-then-extend pattern this entry point existed for.
    pub fn apply_uncached(&mut self, pool: &mut dyn TermPool, term: &Rc<Term>) -> Rc<Term> {
        self.apply(pool, term)
    }
    ...
    fn rename_binding_list<V: BindingValue>(
        ...
        let new_binding_list = binding_list
            ...
            .map(|(var, value)| {
                ...
                // We also need to apply the renaming substitution to each variable's value
