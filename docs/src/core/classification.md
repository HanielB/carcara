...
|---|---|---|---|
| `bind` | **α-renaming instances** (what the rule is for): Skolemize *both* sides at the *same* witnesses and join them — `sko_forall` compares an anchor's witnesses with its own only up to α-equivalence, so the witnesses of `(∀x̄.φ)` serve for `(∀ȳ.ψ)` too and both sides Skolemize to the identical term. **Rewriting instances**: the ∀-ε-clause of one side, `forall_inst` of the other at the same witnesses, and a replay of the body with the witnesses substituted for the anchor's variables (core rules are schematic, so their instances survive a uniform substitution of closed terms), in two directions closed by iff-introduction | **4 steps + 2 anchors, independent of the body** (α-renaming); 2·\|body\| + ~10 + 2 anchors (rewriting) | **done** (`core-expensive` pass, 2026-08-27) — the admissibility argument of the "what the generalization buys" section, made executable. It buys no checking power, so the default regimes keep the rule. Not covered: non-`∀` binders, a *rewriting* `bind` under an enclosing anchor (its replay would have to compose the two substitutions), bodies containing a `let` (substitution renames its bindings independently of their surroundings) or rebinding a substituted variable. 67% of the corpus's `bind` steps reduce, nearly all by the four-step route |
