...
| `ite_intro` | per ite-subterm, the selection tautology `(ite c (= s r₁) (= s r₂))` comes from the term-`ite` selection axioms: `ite_then_intro` is `(cl ¬c (= s r₁))` and `ite_else_intro` is `(cl c (= s r₂))` outright, crossed with `ite_neg1/2` — five steps, subproof-free, plus `refl` + `cong` + `equiv_pos2` for an equality the rule wrote flipped (veriT's usual orientation) — and the equivalence is packed by `and_neg`/`and_pos` + iff-introduction — **fallback implemented** (`core` pass; the output checks with no RARE rules — the earlier two-branch discharge over `rare_rewrite` `ite-true-cond`/`ite-false-cond` needed `--rare-file rare-tests/rare/ite-intro.rare`, and cost ~31 steps per tautology against ~13–20 now) | artifact of veriT's internal ite constants (the spec's own remark); source of the ite-reordering polyeq quirk; removal still preferred |
| `bfun_elim` | both of the rule's steps, at any position of the premise — **fallback implemented** (`core` pass). *Second step* (the below-uninterpreted-functions ite expansion, an equivalence): per `ite` node of the conclusion's tree, one subproof-free branch per polarity — `equiv_neg1/2` + the `true`/`false` axioms give the conditional literal `(cl (= c ⊤) ¬c)`, the clausal `eq_congruent` carries it into the application, `ite_then_intro`/`ite_else_intro` select the branch, `eq_transitive` chains them, and the branches resolve on the condition. *First step* (quantifier expansion): at the top of the premise an implication suffices — `forall_inst` at each of the `2^k` Boolean assignments + `and_neg` repack + closing `bind` over the rest (~8 steps for `k = 1`) — and at any position a congruence has to carry it, so the equivalence is derived: the ← direction takes each instance out of the packing with `and_pos` and turns it back into the body by a case split whose hypotheses are *assumed* — that split is on the anchor's own variables, so nothing in it is context-free and the subproof costs no sharing; the unit hypotheses let plain `cong` carry the rewriting and the vanilla `bind` carry it through a binder of the body — the `2^k` branches resolving on the Boolean variables and closing with the generalized `bind` (~43 steps). An `exists` goes through the quantifier duality with the instances packed as `(not (or φ[σ]))` — `or_pos`/`or_neg` in place of `and_neg`/`and_pos` — which turns back into a disjunction with no De Morgan step (~62 steps). `cong`/`bind` carry the equalities to their positions and `equiv_pos2` crosses them with the implication. Kept: expansions under a `let` or a `choice`/`lambda` (`bind` over `choice` is what the core leaves out), and a conclusion only *polyeq*-equal to the expansion | O(2^k) in the number of Boolean arguments — fails R1; removal preferred. The second step adds no exponential of its own: its `2^j` ite tree is already in the rule's conclusion, and the reduction is linear in it. Using the *clausal* equality rules rather than `cong`/`trans` is what keeps the second step's case splits subproof-free — the hypothesis stays a literal instead of needing an assumption — and so hoistable when the application is ground; the reduction's repeated leaves (`refl`, the conditional literals, the `true`/`false` axioms) are built once per step, which takes the three-argument example from 147 to 99 steps. veriT preprocessing artifact; polyeq elaboration normalizes but keeps it |
...
| `poly_simp` | arithmetic | **expensive** (computational) | ring-normalization primitive, listed in the arithmetic expensive table above: its linear instances reduce to two Farkas bounds, and a consumer that would rather not implement ring normalization can have a proof without it |
